...
def classify_trace_change(
    ...
    """Classify a trace change using the fixture's semantic expectation policy."""
    ...
    expectations = (
        None
        if fixture.expectations is None
        else FixtureExpectations.from_json(fixture.expectations)
    )
    if expectations is not None:
        assertions = _evaluate_expectations(fixture.normalized(), expectations, after)
        if any(
    ...
    if fixture.metadata.severity is FixtureSeverity.CALIBRATION:
        return RegressionReport(
    ...
    return RegressionReport(
        ...
        diff,
        "semantic output changed while gated assertions still pass",
    )
