...
// Battle Brothers' Squirrel bitwise operators use signed 32-bit integers and
// `>>` is arithmetic. SHA-256 requires logical unsigned right shifts. Keep the
// canonical/frame format unchanged while making those shifts explicit.
...
{
    return this._u32(
        this._ushr(this._u32(_value), _bits)
        | this._u32(this._u32(_value) << (32 - _bits))
    );
};

wire.sha256 = function(_message)
{
    local h = [
    ...
    for (local offset = 0; offset < bytes.len(); offset += 64)
    {
        ...
        {
            local s0 = this._rotr(w[i - 15], 7)
                ^ this._rotr(w[i - 15], 18)
                ^ this._ushr(this._u32(w[i - 15]), 3);
            local s1 = this._rotr(w[i - 2], 17)
                ^ this._rotr(w[i - 2], 19)
                ^ this._ushr(this._u32(w[i - 2]), 10);
            w[i] = this._add32(
                this._add32(w[i - 16], s0),
                this._add32(w[i - 7], s1)
            );
        }
