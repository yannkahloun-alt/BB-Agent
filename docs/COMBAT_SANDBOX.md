...
After the deferred projection is available, DEBUG_ORACLE may enqueue a bounded validation plan. Production still uses zero native per-destination pathfinder calls. Native comparisons are diagnostic-only and are processed as ordinary sandbox jobs. When a sampled native path disagrees with the model's tile-count, endpoint, or cost summary, Phase-D diagnostics retain that already-selected native path and sweep one AP prefix budget per later update. The single complete multi-step `farthest_reachable` sample may also run as a control when geometry agrees, so live verification does not depend on reproducing a randomized tactical layout. Distinct cost-prefix anchors reconstruct the ordered native route against canonical player-legal `neighbor_ids`; no navigator internals, command execution, custom native path choice, or oracle-to-production data flow is used.
